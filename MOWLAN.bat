...
start adb shell "logcat -b radio -b main -b events -b system -v time > /data/local/tmp/MOWLAN_android.log &"
adb wait-for-device
...
adb push test_MO_WLAN /data/local/tmp
adb push "D:\My tool\IAC_SQA_MOMTWLAN\UiAutomator2.jar" /data/local/tmp
adb shell "sh /data/local/tmp/test_MO_WLAN > /data/local/tmp/test_MO_WLAN.log &"
pause
